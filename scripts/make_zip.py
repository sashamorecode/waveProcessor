...
def  main():
  ...
  installer = "\installer\waveProcessor Installer.exe"
   
  if demo:
    installer = "\installer\waveProcessor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\waveProcessor manual.pdf" 
  ]
  ...
  ZIPNAME = "waveProcessor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "waveProcessor-v" + FULLVERSIONSTR + "-win-demo.zip"
